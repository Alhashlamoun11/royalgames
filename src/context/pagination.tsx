  ...
  return (
    <div className="pagination__wrap">
      <ul className="list-wrap d-flex flex-wrap justify-content-center">
        {arr.map((item,i) => (
          <li key={i} ><span onClick={(e) =>(currentPage !== item+1)?setCurrentPage(item + 1):console.log(item+1)} style={{cursor:`${currentPage !== item + 1 ? "pointer" : ''}`}} className={`page-numbers ${currentPage === item + 1 ? "current" : ''}`}>{item + 1}</span></li>
        ))}
        <li>
